...
do
    ...
    if [ "${basis}" = "def2QZVP" ]; then
      cat orca6-def2JK.inp | sed "s/{FUNC}/${FUNC}/; s/{chrg}/${charge}/; s/{mult}/${mult}/; s/{coord}/${struct}/; s/{BASIS}/def2-QZVP/; s/{BASISHELP}//" | tr "|" "\n" > ${REALNAME}/${strname}.inp
    elif [ "${basis}" = "def2QZVP+def2ECP" ]; then
      cat orca6-def2JK.inp | sed "s/{FUNC}/${FUNC}/; s/{chrg}/${charge}/; s/{mult}/${mult}/; s/{coord}/${struct}/; s/{BASIS}/def2-QZVP/; s/{BASISHELP}//" | tr "|" "\n" > ${REALNAME}/${strname}.inp
    elif [ "${basis}" = "def2QZVP-WATER27" ]; then
      cat orca6-def2JK-WATER27.inp | sed "s/{FUNC}/${FUNC}/; s/{chrg}/${charge}/; s/{mult}/${mult}/; s/{coord}/${struct}/; s/{BASIS}/def2-QZVP/;" | tr "|" "\n" > ${REALNAME}/${strname}.inp
    elif [ "${basis}" = "aug'-def2QZVP" ]; then
      cat orca6-def2JK-BASIS.inp | sed "s/{FUNC}/${FUNC}/; s/{chrg}/${charge}/; s/{mult}/${mult}/; s/{coord}/${struct}/; s|{BASISHELP}|maug-def2QZVP.obs|" | tr "|" "\n" > ${REALNAME}/${strname}.inp
    else
      echo "Unknown basis"
    fi
done
